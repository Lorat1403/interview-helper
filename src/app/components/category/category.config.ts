...
export const MOCK_DATA: MockDataStructure = {
  angular: [
    ...
    },
    {
      ...
      answer: 'Для встановлення Angular CLI (Command Line Interface) вам потрібен встановлений Node.js і npm (Node Package Manager). Ви встановлюєте Angular CLI глобально на вашій системі за допомогою команди npm install -g @angular/cli у терміналі або командному рядку. CLI є ключовим інструментом для ініціалізації, розробки та підтримки Angular-додатків. Після встановлення ви можете використовувати команду ng для виконання різних завдань, таких як створення проєктів, генерація коду та запуск сервера розробки.',
    },
    {
      id: 3,
      question: 'Що таке компонент в Angular?',
      answer:'Компонент є основним будівельним блоком інтерфейсу користувача в Angular-додатку. Він керує частиною екрана, яку Angular називає поданням (view). Кожен компонент складається з трьох ключових частин: TypeScript-класу, який містить логіку, HTML-шаблону, який визначає, що відображається, і CSS-стилів, які контролюють зовнішній вигляд. Клас компонента також містить декоратор @Component, який надає метадані, такі як селектор для використання в шаблонах. Компоненти можуть бути вкладеними, що дозволяє створювати складні інтерфейси шляхом комбінування простих компонентів. Вони також підтримують зв’язування даних і обробку подій, що робить їх інтерактивними. Компоненти є ключовою частиною архітектури Angular і сприяють модульності та повторному використанню коду.',
    },
    {
      id: 4,
      question: 'Як створити новий компонент в Angular?',
      answer:'Щоб створити новий компонент в Angular, ви використовуєте команду Angular CLI ng generate component <назва-компонента>. Ця команда автоматично генерує всі необхідні файли для компонента (файл класу .ts, шаблон .html, стилі .css або .scss та файл тестування .spec.ts). Ви також можете використовувати скорочену команду ng g c <назва-компонента>.',
    },
    {
      id: 5,
      question: 'Що таке модуль в Angular?',
      answer: "Модуль в Angular (часто називається NgModule) є контейнером для логічно пов'язаного блоку коду, такого як компоненти, сервіси та інші провайдери. Кожен Angular-додаток має принаймні один кореневий модуль, який зазвичай називається AppModule, що виконує початкове завантаження. Модулі використовуються для організації додатку, декларування того, які компоненти, директиви та пайпи належать до нього, та експортування функціональності для використання іншими модулями. Вони також допомагають Angular зрозуміти, як компілювати код та які функції доступні.",
    },
    {
      id: 6,
      question: 'Що таке директива в Angular?',
      answer: "Директива в Angular — це клас, який дозволяє змінювати поведінку або зовнішній вигляд елементів DOM (Document Object Model). Існують три основні типи директив: структурні директиви, які змінюють структуру DOM (наприклад, *ngIf, *ngFor); атрибутивні директиви, які змінюють вигляд або поведінку елементів (наприклад, ngClass, ngStyle); і компоненти, які є спеціальним типом директив з власним шаблоном. Директиви дозволяють розробникам створювати повторно використовувані функції та покращувати взаємодію користувача з додатком.",
     },
    {
      id: 7,
      question: 'Що таке сервіс в Angular?',
      answer: "Сервіс (Service) в Angular – це клас, призначений для виконання специфічних завдань, які не пов'язані безпосередньо з відображенням даних, наприклад, отримання даних із сервера, логування або обмін даними між компонентами. Вони дозволяють відокремити логіку обробки даних від логіки відображення, роблячи код чистішим та повторно використовуваним. Сервіси зазвичай надаються (реєструються) в системі впровадження залежностей (Dependency Injection) Angular, що дозволяє компонентам легко їх використовувати.",
    },
    {
      id: 8,
      question: 'Що таке двостороннє зв’язування даних?',
      answer: "Двостороннє зв'язування даних (Two-Way Data Binding) – це механізм в Angular, який забезпечує автоматичну синхронізацію між даними в моделі (властивості в класі компонента) і поданням (значенням елемента форми в HTML-шаблоні). Коли дані змінюються в моделі, вони автоматично оновлюються в поданні, і навпаки: коли користувач змінює дані в елементі введення (наприклад, у полі input), це значення автоматично оновлює відповідну властивість у моделі",
    },
    {
      id: 9,
      question: 'Що таке шаблон в Angular?',
      answer: "Шаблон в Angular – це HTML-код, який визначає, як має виглядати інтерфейс користувача. Шаблони можуть містити Angular-специфічні синтаксиси, такі як директиви та вирази, які дозволяють інтерактивно відображати дані з компонентів. Шаблони можуть бути простими або складними, включаючи компоненти, структурні директиви (наприклад, *ngIf, *ngFor) та атрибутивні директиви (наприклад, ngClass, ngStyle). Вони є важливою частиною архітектури Angular, оскільки відокремлюють логіку відображення від бізнес-логіки додатку.",
    },
    {
      id: 10,
      question: 'Як працює маршрутизація в Angular?',
      answer: "Маршрутизація в Angular – це механізм, який дозволяє навігацію між різними компонентами або сторінками в односторінковому додатку (SPA). Вона реалізується за допомогою Angular Router, який керує URL-адресами та відображенням відповідних компонентів на основі цих адрес. Ви визначаєте маршрути в конфігурації маршрутизатора, вказуючи шлях (URL) і відповідний компонент. Коли користувач переходить за певним URL, Angular Router завантажує відповідний компонент і відображає його у визначеному місці в шаблоні (зазвичай за допомогою <router-outlet>). Маршрутизація також підтримує параметри маршрутів, захист маршрутів (route guards) та lazy loading для оптимізації продуктивності додатку.",
    },
    {
      id: 11,
      question: 'Як додати стиль до компонента в Angular?',
      answer: "Щоб додати стиль до компонента в Angular, ви можете використовувати CSS-файли, які пов'язані з компонентом, або вбудовані стилі. Angular підтримує різні способи стилізації компонентів, включаючи використання глобальних стилів, стилів, специфічних для компонента, та стилів, які застосовуються на основі умов (наприклад, за допомогою директиви ngClass). Ви також можете використовувати Angular Material для швидкого створення стильних інтерфейсів.",
    },
    {
      id: 12,
      question: 'Що таке інтерполяція в Angular?',
      answer: "Інтерполяція в Angular – це механізм, який дозволяє вставляти значення змінних або виразів з класу компонента безпосередньо в HTML-шаблон. Вона використовується для відображення динамічних даних у вигляді тексту. Інтерполяція здійснюється за допомогою подвійних фігурних дужок {{ }}. Наприклад, якщо у вас є властивість name в класі компонента, ви можете відобразити її значення в шаблоні за допомогою {{ name }}. Angular автоматично оновлює відображення, коли значення властивості змінюється.",
    },
    {
      id: 13,
      question: 'Що таке *ngIf директива?',
      answer: "Директива *ngIf в Angular є структурною директивою, яка дозволяє умовно відображати або приховувати елементи в DOM на основі логічного виразу. Якщо вираз оцінюється як true, елемент і його вміст будуть додані до DOM; якщо false – елемент буде видалений з DOM. Це корисно для керування видимістю частин інтерфейсу користувача залежно від стану додатку або даних. Використання *ngIf допомагає оптимізувати продуктивність, оскільки елементи, які не потрібні, не створюються в DOM взагалі.",
    },
    {
      id: 14,
      question: 'Що таке *ngFor директива?',
      answer: "Директива *ngFor в Angular є структурною директивою, яка використовується для ітерації (перебору) колекцій, таких як масиви або списки, і створення елементів у DOM для кожного елемента в цій колекції. Вона дозволяє динамічно генерувати вміст на основі даних. Синтаксис *ngFor включає ключове слово let для оголошення змінної, яка представляє поточний елемент у колекції, а також ключове слово of для вказівки самої колекції. Наприклад, *ngFor=\"let item of items\" створить елемент для кожного item у масиві items. Це дуже корисно для відображення списків даних у користувацькому інтерфейсі.",
    },
